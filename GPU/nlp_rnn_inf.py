...
def model_train_save(saved_model_dir,max_review_length,embedding_vector_length,batchsize,top_words=5000):
    ...
    model = Sequential()
    model.add(Embedding(top_words, embedding_vector_length, input_length = len(X_train[0])))
    model.add(SimpleRNN(16, input_shape=(top_words, max_review_length), return_sequences=False, activation="relu"))
