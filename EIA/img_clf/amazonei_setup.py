...
try:
    ...
    logging.error(
        "This utility requires 'boto3'. Please install it and run the utility again. Command to install boto3 - 'sudo pip install --upgrade boto3'")
    quit()
...
class IAM:
    ...
    def create_instance_profile(self):
        ...
        self._iam_Client.add_role_to_instance_profile(InstanceProfileName=self._wizard_instance_profile_name,
                                                      RoleName=self._wizard_role_name)
        return profile
    ...
    def create_eia_connect_role(self):
        ...
        return self._iam_Client.create_role(AssumeRolePolicyDocument=policy_json, Path='/',
                                            RoleName=self._wizard_role_name)

    def create_eia_connect_policy(self):
        policy_json = json.dumps(self._policy_doc)
        return self._iam_Client.create_policy(PolicyName=self._wizard_policy_name, Path='/', PolicyDocument=policy_json,
                                              Description="Policy for connecting to Amazon EI service")
...
class EC2:
    ...
    def launch_instance(self, image_id, instance_type, key_name, security_group, subnet_id, instance_profile,
                        accelerator_type):
    ...
    def create_endpoint(self, region_name, vpc_id, security_group, subnet_id):
        ...
        if len(response['VpcEndpoints']) == 0:
            ...
            print("\n Discovered VPC endpoint for Amazon EI service, ID: {}".format(
...
class UserInput:
    ...
    def get_session(self):
        ...
        if self.is_ei_service_available(self._region_name) == False:
            raise Exception(
                "\n Amazon Elastic Inference service is not available in the specified region: '{}' ".format(
                    self._region_name))
    ...
    def get_accelerator_type(self):
        ...
        accel_memory = ['2 GB of accelerator memory', '4 GB of accelerator memory', '8 GB of accelerator memory',
                        '1 GB of accelerator memory', '2 GB of accelerator memory', '4 GB of accelerator memory']
    ...
    def get_subnet(self, vpc_id):
        ...
        if len(subnets) == 0:
            print(
                "\nNo subnet found. Please create a Subnet in one of the following AZs for VPC ID: {}, and run this utility again:".format(
                    vpc_id))
        ...
        if len(subnets) > 1:
            ...
            for subnet_index, subnet in enumerate(subnets):
                print(" {}: SubnetID '{}', AvailabilityZone '{}'".format(subnet_index, subnet['SubnetId'],
...
if __name__ == "__main__":
    ...
    if StrictVersion(boto3.__version__) < StrictVersion('1.9.71'):
        logErrorAndQuit(
            "Minimum boto3 version required is 1.9.71. Found version - " + boto3.__version__ + ". Please update it using command - 'sudo pip install --upgrade boto3'")
    ...
    try:
        ...
        if (wizard_role == None):
            ...
            print("\n Found an IAM role configured for connecting to Amazon EI service. Name - {}, ARN - {}".format(
                wizard_role['RoleName'], wizard_role['Arn']))
        ...
        if sg_id == None:
            print(
                " The EC2 instance and the private link endpoint require a security group with TCP port 443 enabled. This script will now create a security group with the required inbound rules.")
            sg_response = user_input.create_security_group('amazon_ei_security_group',
                                                           'Security Group for accessing Amazon EI service', vpc_id,
                                                           443)
        ...
        summary = "\n The script will now launch new instance with following configuration. Type 'y' to continue. \n\n Accelerator Type: {}\n Region: {}\n Image-ID: {} - ({})\n Instance Type: {}\n Key Pair: {}\n Security Group ID: {}\n Subnet ID: {}\n Instance Profile: {}".format \
                (
                accel_type, region, image_id['ImageId'], image_id['Name'], instance_type, key_pair, sg_id, subnet_id,
                iam_client.get_instance_profile_name()
            )
        print(summary)
        confirmation = user_input.get_confirmation()
        if (confirmation == 'y' or confirmation == 'Y'):
            print("\n Launching Instance ..")
            launch_response = ec2_client.launch_instance(image_id['ImageId'], instance_type, key_pair, sg_id, subnet_id,
                                                         iam_client.get_instance_profile_name(), accel_type)
